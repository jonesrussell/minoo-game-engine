...
export function validatePresentation(input: unknown, objectIds?: readonly string[]): PresentationValidation {
  ...
  presentation.characters.forEach((character, index) => {
    ...
    for (const [expression, assetId] of Object.entries(character.expressions)) {
      if (!assetIds.has(assetId)) errors.push({ code: 'PRESENTATION_REFERENCE', path: `/characters/${index}/expressions/${expression}`, message: `Asset "${assetId}" does not exist for expression "${expression}".` });
    }
  });
...
export function renderConversation(presentation: GamePresentation, index: number, button: (id: string, text: string, primary?: boolean, disabled?: boolean) => string): string {
  ...
  return `<div class="conversation-heading"><span>${escapePresentationHtml(presentation.mission.brand)}</span><span>${view.index + 1} / ${view.total}</span></div><div class="conversation-cast" aria-hidden="true">${presentation.characters.map(candidate => { const neutralId = candidate.expressions.neutral; const requestedId = candidate.id === character.id ? (candidate.expressions[expression] ?? neutralId) : neutralId; const requested = presentation.assets.find(asset => asset.id === requestedId); const neutral = presentation.assets.find(asset => asset.id === neutralId); const displayedExpression = candidate.id === character.id ? expression : 'neutral'; return `<div class="character-slot ${escapePresentationHtml(candidate.id)} ${candidate.id === character.id ? 'speaking' : ''}"><img class="character-portrait" data-expression="${displayedExpression}" data-neutral-src="${escapePresentationHtml(neutral?.url ?? '')}" src="${escapePresentationHtml(requested?.url ?? neutral?.url ?? '')}" alt=""><span class="portrait-fallback" hidden>${escapePresentationHtml(candidate.name)}</span></div>`; }).join('')}</div><div class="conversation-strip"><div class="speaker-name">${escapePresentationHtml(character.name)}</div><p class="spoken-line" aria-live="polite">${escapePresentationHtml(view.beat.line)}</p><div class="dialogue-controls"><span class="line-count">${view.index + 1} / ${view.total}</span>${button('dialogue-back', 'Back', false, view.index === 0)}${button('dialogue-skip', 'Skip conversation')}${button('dialogue-next', view.index === view.total - 1 ? 'Find the kit' : 'Next', true)}</div></div>`;
}

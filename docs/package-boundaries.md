...
This document records the ownership decision for issue [#33](https://github.com/jonesrussell/minoo-game-engine/issues/33). The paths below are planned package boundaries. They describe contracts for future implementation; they do not claim that the packages or services already exist.
...
| ID | Requirement |
| --- | --- |
| OWN-001 | Framework contracts are platform-neutral and contain versioned scene, action, content-reference, validation and extension types. |
...
| OWN-006 | The standalone browser game and local authoring workflow run without Studio, an account service, a model call or a network service during ordinary play. |
...
| --- | --- | --- |
| `packages/engine/contracts` | Standard TypeScript/runtime types and small platform-neutral utilities | `engine/core`, `authoring`, `games/*`, browser APIs, Studio or network clients |
| `packages/engine/core` | `engine/contracts` public contracts and platform-neutral utilities | `games/*`, `authoring`, browser APIs, DOM/Canvas, Studio or model clients |
| `packages/engine/adapters` | `engine/contracts` public contracts and `engine/core` public ports | Engine private state, authoring internals, Studio clients or game-specific rules |
| `packages/authoring` | `framework` contracts and documented `runtime` validation/replay ports | Renderer/browser code, runtime private state, Studio account/project code |
| `games/journey` and `games/matcher` | `framework` contracts, documented runtime extension interfaces and their own content/assets | Other game packages, framework internals, renderer internals or Studio services |
...
The exact directory names may change when the workspace is implemented. The dependency direction and public/private distinction are the decision. New dependencies require a concrete use case and an update to this table.
...
Ordinary gameplay must work with local assets and deterministic inputs. It does not require Studio, PHP, an account, an API key, a hosted model, an MCP server or a backend. Optional Studio integration may review and coordinate Minoo candidate artifacts later, but the adapter depends on Minoo and the runtime never calls Studio.
